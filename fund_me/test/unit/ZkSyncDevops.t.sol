...
contract ZkSyncDevops is Test, ZkSyncChainChecker, FoundryZkSyncChecker {
    // Remove the `skipZkSync`, then run `forge test --mt testZkSyncChainFails --zksync`
    function testZkSyncChainFails() public skipZkSync{
        address ripemd = address(uint160(3));
        bool success;

        // Don't worry about assembly for now
        assembly {
    ...
    // Remove the `onlyVanillaZkSync`, then run `foundryup-zksync` and then
